...
function buildApp(config: SocialStackConfig) {
	...
	function get(url: string, raw?: boolean) {
		...
		return fetch(config.instanceUrl + url)
			.then((r: any) => (url.indexOf('.') != -1 || raw) ? r.text() : r.json().then((r: any) => r.results || r.result));
	}

	function post(url: string, body: any, raw?: boolean) {
	...
	function getAsFile(url: string, targetPath: string) {
		if (requiresSlash) {
			url = '/' + url;
		}
	...
	function saveFile(filepath: string, fileContent: string) {

		var normPath = path.normalize(dir + '/' + filepath);

		return new Promise<void>((s, r) => {

			fs.mkdir(path.dirname(normPath), { recursive: true }, (err: any) => {
				if (err) {
					return r(err);
				}

				fs.writeFile(normPath, fileContent, { encoding: 'utf8' }, (err: any) => {
	...
	function configNotFound(platform: string) {
		throw new Error(
	...
	function compareAsset(asset: any, targetPath: string) {
		...
		return new Promise<void>((s, r) => {
			...
			fs.mkdir(path.dirname(normPath), { recursive: true }, (err: any) => {
				...
				fs.stat(normPath, (err: any, stat: any) => {
	...
	var prom = get('v1/locale/list', true)
		.then((localeJson: any) => {
			...
			var promisesJs = locales.map((locale: any) => get('pack/main.js?lid=' + locale.id).then((fileContent: string) => saveFile(buildConfig.directory + '/pack/main.' + locale.code + '.js', fileContent)));
			var promisesCss = locales.map((locale: any) => get('pack/main.css?lid=' + locale.id).then((fileContent: string) => saveFile(buildConfig.directory + '/pack/main.' + locale.code + '.css', fileContent)));
		...
		.then((locales: any[]) => {
			...
			var promisesHtml = locales.map((locale: any) => {

				return post('pack/static-assets/mobile-html', { localeId: locale.id, apiHost: config.apiUrl, customJs: '{{APP_JS_SUBSTITUTE}}' }, true).then((html: string) => {
