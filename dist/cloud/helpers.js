...
const { jsConfigManager, getLocalConfig, setLocalConfig } = configManager_1.default;
...
function hasGroupConfig(dirPath, callback) {
    ...
    fs_1.default.stat(dirPath + '/socialstackCloud.json', fileReturn);
}
/*
    ...
    return new Promise((s, r) => {
        ...
        function onCheckedRoot(success) {
            if (success) {
                ...
                intermediateDirNames.push(path_1.default.basename(currentPath));
...
function askFor(text, configName, configSet, muted) {
    return new Promise((success, reject) => {
        ...
        var rl = readline_1.default.createInterface({ input: process.stdin, output: mutableStdout, terminal: true });
...
function tryLogin(userAuth, host) {
    return (0, node_fetch_1.default)(host + '/v1/user/login', {
        method: 'POST',
        headers: {
        ...
        if (json.message) {
            ...
            // 2FA
            return askFor("Two factor authentication is enabled on this account. Please provide the current pin.", "google2FAPin", userAuth).then(() => tryLogin(userAuth, host));
        }
...
function get(url, config) {
    return (0, node_fetch_1.default)(config.host + '/v1/' + url, {
        headers: {
            'Token': config.token
        }
    });
...
function startCloudDeployment(config) {
    ...
    getAuth()
        ...
        return askFor('How many people approx will be using this site at the same time? Just press enter to get a default single server instance.', 'peopleExpected', project);
    })
        .then(() => {
        ...
        return findGroupConfig(config)
            ...
            if (groupAndIntermediateDirs.groupPath != null) {
                ...
                console.log('Didn\'t find any socialstackCloud.json files. Placing one of these in a parent directory to the project means the project ' +
