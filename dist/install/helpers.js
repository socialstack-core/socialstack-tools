...
function writeMeta(localModulePath, remoteModule) {
    ...
    fs_1.default.writeFileSync(path_1.default.join(localModulePath, 'module.installer.json'), meta);
}
function isModuleDifferent(localModulePath, moduleId, versionCode) {
    return getOrCacheZip({
        ...
        return new Promise((success, reject) => {
            zipStream.pipe(unzipper_1.default.Parse()).on('entry', function (entry) {
                if (entry.type == 'File') {
                    pend++;
                    var localPath = path_1.default.join(localModulePath, entry.path);
                    var localRead = fs_1.default.createReadStream(localPath);
...
function searchForModules(dirPath, results, depth) {
    ...
    if (fs_1.default.existsSync(dirPath)) {
        fs_1.default.readdirSync(dirPath).forEach((file, index) => {
            if (!depth && (file == 'bin' || file == '.git')) {
                return;
            }
            var curPath = path_1.default.join(dirPath, file);
            if (fs_1.default.lstatSync(curPath).isDirectory()) { // recurse
                ...
                if (file == 'module.installer.json') {
                    var meta = fs_1.default.readFileSync(curPath, { encoding: 'utf8' });
                    try {
...
function getOrCacheZip(moduleMeta) {
    ...
    return new Promise((success, reject) => {
        ...
        fs_1.default.mkdir(moduleCachePath, { recursive: true }, (err) => {
            if (err && err.code != 'EEXIST')
                throw err;
            var zipPath = '/' + moduleMeta.latestVersionCode + '.zip';
            var readStream = fs_1.default.createReadStream(moduleCachePath + zipPath);
            ...
            readStream.on('error', function (err) {
                ...
                https_1.default.get(url, function (response) {
                    if (response.statusCode == 200 && response.headers['content-type'] == 'application/zip') {
                        var cacheWriteStream = fs_1.default.createWriteStream(moduleCachePath + zipPath + ".tmp");
                        response.pipe(cacheWriteStream);
                        cacheWriteStream.on('finish', () => {
                            // Delete if it exists:
                            try {
                                fs_1.default.unlinkSync(moduleCachePath + zipPath);
                            }
                            catch { }
                            // Move the tmp file.
                            fs_1.default.renameSync(moduleCachePath + zipPath + ".tmp", moduleCachePath + zipPath);
                            // try opening again:
                            readStream = fs_1.default.createReadStream(moduleCachePath + zipPath);
                            readStream.on('open', function () {
                                success(readStream);
                            });
                            readStream.on('error', function (err) {
...
function uninstallModules(modules, config) {
    return new Promise((success, reject) => {
        var projectRoot = path_1.default.normalize(config.projectRoot);
...
function installSingleModuleInternal(moduleMeta, moduleFilePath, config, addMeta, dependencySkipMap) {
    ...
    return getOrCacheZip(moduleMeta).then(zipStream => {
        ...
        return new Promise((success, reject) => {
            ...
            var delPromise;
            if (addMeta) {
                delPromise = (0, rimraf_1.default)(moduleFilePath);
            }
            else {
            ...
            delPromise.then(() => {
                ...
                zipStream.pipe(unzipper_1.default.Parse()).on('entry', function (entry) {
                    if (entry.type == 'File') {
                        mkDirByPathSync(moduleFilePath + path_1.default.dirname(entry.path));
                        entry.pipe(fs_1.default.createWriteStream(moduleFilePath + entry.path));
                    }
                    else {
...
function getNewerVersions(moduleId, version) {
    return new Promise((success, reject) => {
        https_1.default.get('https://' + moduleRepository + '/v1/module/' + moduleId + '/newer-versions?versionCode=' + version, function (res) {
            var bodyResponse = [];
            res.on('data', (d) => {
                bodyResponse.push(d);
            });
...
function getModuleList() {
    ...
    return new Promise((success, reject) => {
        https_1.default.get('https://' + moduleRepository + '/v1/module/list', function (res) {
            var bodyResponse = [];
            res.on('data', (d) => {
                bodyResponse.push(d);
            });
...
function installDependencies(moduleFilePath, config, dependencySkipMap) {
    return new Promise((success, reject) => {
        fs_1.default.readFile(moduleFilePath + 'package.json', { encoding: 'utf8' }, (err, content) => {
            if (err) {
            ...
            try {
                ...
                if (pkg && Array.isArray(pkg.dependencies)) {
                    pendingPromises.push(installModules(pkg.dependencies, config, dependencySkipMap));
                }
