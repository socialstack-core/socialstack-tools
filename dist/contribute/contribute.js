...
const { runCmd } = helpers_js_1.default;
const rimraf_1 = __importDefault(require("rimraf"));
const readline_1 = __importDefault(require("readline"));
const helpers_js_2 = __importDefault(require("../install/helpers.js"));
const { getModuleMap, getModuleIdMap, searchForModules, isModuleDifferentAndCorrect, getModuleFilePath, writeMeta } = helpers_js_2.default;
...
function applyCommit(commitInfo, repoConfig) {
    ...
    var target = path_1.default.join(repoConfig.repoCheckout, remoteModule.path);
    var src = localModule.path;
    var gitConfig = { projectRoot: repoConfig.repoCheckout };
    return (0, rimraf_1.default)(target)
        .then(() => {
...
var copyRecursiveSyncExceptForModuleJson = function (src, dest, depth) {
    ...
    var stats = exists && fs_1.default.statSync(src);
    ...
    if (isDirectory) {
        ...
        fs_1.default.readdirSync(src).forEach(function (childItemName) {
            ...
            copyRecursiveSyncExceptForModuleJson(path_1.default.join(src, childItemName), path_1.default.join(dest, childItemName), depth + 1);
        });
    }
    else {
...
exports.default = (config) => {
    ...
    fs_1.default.mkdir(repoCheckout, { recursive: true }, (err) => {
        if (err && err.code != 'EEXIST')
            throw err;
        var repoExists = null;
        try {
            repoExists = fs_1.default.statSync(repoCheckout + '/.git');
        }
        catch {
        }
