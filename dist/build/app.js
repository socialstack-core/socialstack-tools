...
function buildApp(config) {
    ...
    function get(url, raw) {
        ...
        return (0, node_fetch_1.default)(config.instanceUrl + url)
    ...
    function getAsFile(url, targetPath) {
        ...
        return (0, node_fetch_1.default)(config.instanceUrl + url)
            ...
            return streamPipeline(response.body, fs_1.default.createWriteStream(targetPath));
        });
    }
    function saveFile(filepath, fileContent) {
        var normPath = path_1.default.normalize(dir + '/' + filepath);
        return new Promise((s, r) => {
            fs_1.default.mkdir(path_1.default.dirname(normPath), { recursive: true }, (err) => {
                if (err) {
                    return r(err);
                }
                fs_1.default.writeFile(normPath, fileContent, { encoding: 'utf8' }, (err) => {
                    if (err) {
                        return r(err);
                    }
                    s();
    ...
    function configNotFound(platform) {
        throw new Error('Config does not exist in appsettings for platform "' + platform +
    ...
    function compareAsset(asset, targetPath) {
        return new Promise((s, r) => {
            ...
            var normPath = path_1.default.normalize(dir + '/' + targetPath);
            fs_1.default.mkdir(path_1.default.dirname(normPath), { recursive: true }, (err) => {
                if (err) {
                    return r(err);
                }
                fs_1.default.stat(normPath, (err, stat) => {
    ...
    var prom = get('v1/locale/list', true)
        ...
        */
        ...
        appSpecificJs += 'var contentSource="' + config.apiUrl + '";' + fs_1.default.readFileSync(__dirname + "/appFrontend.js", { encoding: 'utf8' });
