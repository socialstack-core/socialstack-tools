...
const { remoteFileList, localFileList, diff, copyDirectory, uploadFile, createRemoteDirectory, extractPatch, setPermsAndUser, setupOrRestartService, handleRenames, getAppSettings, reloadUI } = helpers_js_1.default;
const helpers_js_2 = __importDefault(require("../host/helpers.js"));
const helpers_js_3 = __importDefault(require("../cloud/helpers.js"));
const helpers_js_4 = __importDefault(require("../build/helpers.js"));
const tmp_1 = __importDefault(require("tmp"));
const tar_1 = __importDefault(require("tar"));
const path_1 = __importDefault(require("path"));
function deployToHost(host, config, verbose, uiOnly, content, commit) {
    return helpers_js_2.default.connect(host)
        .then(connection => {
        ...
        if (content) {
            fileSets.push({ name: 'Content', local: 'Content', remote: 'Content', noRemovals: true }, { name: 'Database', local: 'Database', remote: 'Database', onCheckExclude: (file) => {
        ...
        var setPromises = fileSets.map(fileSet => {
            return Promise.all([
                ...
                if (patch.added.length || patch.updated.length) {
                    // Temp path first:
                    var tmpFile = tmp_1.default.tmpNameSync();
                    ...
                    fileSetPatch.compressedPatch = tar_1.default.create({
                        gzip: true,
                        file: tmpFile,
                        cwd: path_1.default.resolve(config.projectRoot, fileSet.local)
                    }, filesToUpload).then(() => fileSetPatch);
                }
                else {
...
exports.default = config => {
    if (config.commandLine['h'] || config.commandLine['host']) {
        ...
        var buildPromise = build ? helpers_js_4.default.buildAll({ prod: true, noApi: uiOnly, bundled }, config) : Promise.resolve(true);
        buildPromise = buildPromise.catch(e => {
            e && e.message ? console.log(e.message) : console.error(e);
            return;
        });
        // Wait for build (if there even is one):
        return buildPromise.then(() => {
            // Deploy to each host:
            return Promise.all(hosts.map(host => deployToHost(host, config, verbose, uiOnly, content, commit)));
        });
    }
    else {
        // Cloud deployment
        return helpers_js_3.default.runCmd(config);
    }
};
