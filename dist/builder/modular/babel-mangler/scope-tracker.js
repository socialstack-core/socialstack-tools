...
const counted_set_1 = __importDefault(require("./counted-set"));
...
class ScopeTracker {
    ...
    addScope(scope) {
        if (!this.references.has(scope)) {
            this.references.set(scope, new counted_set_1.default());
        }
    ...
    addReference(scope, binding, name) {
        let parent = scope;
        do {
            var parentSet = this.references.get(parent);
            if (!parentSet) {
                this.addScope(parent);
                parentSet = this.references.get(parent);
            }
            parentSet.add(name);
            if (!binding) {
    ...
    canUseInReferencedScopes(binding, next) {
        ...
        for (let i = 0; i < binding.referencePaths.length; i++) {
            ...
            else if (!(0, is_label_identifier_1.default)(ref)) {
