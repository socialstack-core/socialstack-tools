...
class NpmBundle {
    ...
    constructor(projectRoot, minified, globalMap) {
        ...
        this.packDir = path_1.default.resolve(projectRoot, 'UI', 'public', 'pack');
    }
    /// <summary>
    ...
    addRequire(filePath, relativeTo) {
        ...
        return new Promise((success, reject) => {
            if (filePath.startsWith('.') || filePath.startsWith('..')) {
                path_1.default.resolve(relativeTo, filePath);
            }
            else {
                // Figure out which file is the root of this module.
                // Read its package.json first:
                var fullModulePath = relativeTo + filePath;
                this.loadTextFile(path_1.default.resolve(fullModulePath, 'package.json')).then(packageJson => {
                    var loadedPkg = JSON.parse(packageJson);
                    var moduleRoot = loadedPkg.module || "esm";
                    var fStat;
                    var filePath;
                    try {
                        filePath = path_1.default.resolve(fullModulePath, moduleRoot);
                        fStat = fs_1.default.statSync(filePath);
                        if (fStat.isDirectory()) {
                            filePath = path_1.default.resolve(modRoot, 'index.js');
                            fStat = fs_1.default.statSync(filePath);
                        }
                    }
                    catch {
                        ...
                        filePath = path_1.default.resolve(fullModulePath, loadedPkg.main || 'index.js');
                        try {
                            fStat = fs_1.default.statSync(filePath);
                        }
                        catch {
                            throw new Error("Unable to require '" + filePath + "' as it doesn't appear to exist. " +
    ...
    outputFile(fileName, content) {
        ...
        fs_1.default.writeFileSync(this.packDir + '/' + fileName, content, { encoding: 'utf8' });
    }
    /// <summary>
