...
const js_transforms_js_1 = require("./js-transforms.js");
const css_transforms_js_1 = require("./css-transforms.js");
const mkdir_recursive_js_1 = __importDefault(require("../buildwatch/mkdir-recursive.js"));
const copyStaticFiles_js_1 = __importDefault(require("./copyStaticFiles.js"));
class UIBundle {
    /// <summary>
    ...
    constructor(rootPath, projectRoot, globalFileMap, minified, cache) {
        ...
        this.rootPath = path_1.default.resolve(projectRoot, rootPath);
        this.sourcePath = path_1.default.resolve(projectRoot, rootPath, "Source");
        this.globalFileMap = globalFileMap;
        this.cache = cache; // UIBuildCache (never null)
        this.containsStarterModule = false;
        this.minified = minified;
        this.packDir = rootPath == 'Admin' ? path_1.default.resolve(projectRoot, rootPath, 'public', 'en-admin', 'pack') : path_1.default.resolve(projectRoot, rootPath, 'public', 'pack');
    }
    /// <summary>
    ...
    copyStaticFiles() {
        ...
        for (var k in this.staticFileMap) {
            ...
            proms.push((0, copyStaticFiles_js_1.default)(staticFile.path, staticFile.targetPath, true));
        }
    ...
    ensureTarget() {
        return new Promise((s, r) => {
            (0, mkdir_recursive_js_1.default)(this.packDir, err => {
                err ? r() : s();
            });
        });
    }
    ...
    buildEverything() {
        ...
        for (var key in this.fileMap) {
            ...
            if (file.fileType == SourceFileType.Javascript) {
                buildProms.push(this.buildJsFile(file, cachedFile));
            }
            else if (file.fileType == SourceFileType.Scss && !file.isGlobal) {
                if (this.globalFileMap.hasChanges) {
                    cachedFile = null;
                }
                buildProms.push(this.buildScssFile(file, cachedFile));
            }
        }
    ...
    outputFile(fileName, content) {
        var outputPath = path_1.default.resolve(this.packDir + '/' + fileName);
        console.log("Outputting '" + fileName + "' to: ", outputPath);
        fs_1.default.writeFileSync(outputPath, content, { encoding: 'utf8' });
    }
    /// <summary>
            ...
            for (var i = 0; i < pathParts.length; i++) {
                ...
                if (pathPart == ".") {
                    ...
                    if (builtPath.length < 1) {
                        throw new Error("The source path '" + sourcePath + "' in a css file is referring to a file outside the scope of the source directory. It was in " + filePathParts.join('/'));
                    }
                    builtPath.pop();
                }
                else {
    ...
    transformScssFile(file) {
        ...
        var transpiledCss = (0, css_transforms_js_1.transform)(rawContent, header, file.path, this.minified);
        // Convert URLs:
    ...
    transformJsFile(file) {
        ...
        var es8JavascriptResult = (0, js_transforms_js_1.transformES8)(file.content, file.modulePath, // Module path
        file.fullModulePath, { minified: this.minified });
        // Get the src:
