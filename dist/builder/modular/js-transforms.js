...
const index_js_1 = __importDefault(require("./babel-mangler/index.js"));
...
function transformImport(nodePath, state) {
    ...
    if (importedFrom.startsWith('s:')) {
        ...
        nodePath.replaceWith(state.types.variableDeclaration("var", [state.types.variableDeclarator(targetLocal, state.types.stringLiteral(importedFrom))]));
        return;
    }
    ...
    if (sourcePath == "react") {
        ...
        requireStatement = state.types.callExpression(state.types.identifier("_rq"), [state.types.stringLiteral(importedFrom)]);
    }
    if (specifiers && specifiers.length) {
        ...
        nodePath.replaceWith(requireStatement);
    }
}
function toExpression(declaration, state) {
    if (state.types.isFunctionDeclaration(declaration)) {
        return state.types.functionExpression(declaration.id, declaration.params, declaration.body, declaration.generator, declaration.async);
    }
    else if (state.types.isClassDeclaration(declaration)) {
        return state.types.classExpression(declaration.id, declaration.superClass, declaration.body, declaration.decorators);
    }
...
function transformExport(nodePath, state) {
    ...
    if (state.types.isExportNamedDeclaration(nodePath)) {
        ...
        if (declaration) {
            if (state.types.isVariableDeclaration(declaration)) {
                ...
                for (var i = 0; i < thingsExported.length; i++) {
                    ...
                    replacement.push(state.types.expressionStatement(state.types.assignmentExpression('=', state.types.memberExpression(state.types.identifier("exports"), state.types.identifier(thingExported.id.name) // must make a new identifier to avoid it mangling the exported name
                    ), thingExported.init)));
                }
                ...
                nodePath.replaceWithMultiple([
                    ...
                    declaration,
                    // The exports line:
                    state.types.expressionStatement(state.types.assignmentExpression('=', state.types.memberExpression(state.types.identifier("exports"), state.types.identifier(declaration.id.name)), declaration.id))
                ]);
            }
        }
        else {
            ...
            if (nodePath.node.source) {
                ...
                if (importedFrom.startsWith('s:')) {
                    ...
                    replacement.push(state.types.expressionStatement(state.types.assignmentExpression('=', state.types.memberExpression(state.types.identifier("exports"), state.types.identifier(targetExport)), state.types.stringLiteral(importedFrom))));
                }
                else {
                    // some other module. Require it first:
                    var requireStatement = state.types.expressionStatement(state.types.assignmentExpression('=', state.types.identifier("_eI"), state.types.callExpression(state.types.identifier("_rq"), [state.types.stringLiteral(importedFrom)])));
                    replacement.push(requireStatement);
                    for (var i = 0; i < thingsExported.length; i++) {
                        var thingExported = thingsExported[i];
                        var name = thingExported.exported.name;
                        replacement.push(state.types.expressionStatement(state.types.assignmentExpression('=', state.types.memberExpression(state.types.identifier("exports"), state.types.identifier(name)), state.types.memberExpression(state.types.identifier("_eI"), thingExported.local))));
                    }
                }
            }
            else {
                for (var i = 0; i < thingsExported.length; i++) {
                    ...
                    replacement.push(state.types.expressionStatement(state.types.assignmentExpression('=', state.types.memberExpression(state.types.identifier("exports"), state.types.identifier(name)), toExpression(thingExported, state))));
                }
            }
            nodePath.replaceWithMultiple(replacement);
        }
        ...
        if (thingExported.id) {
            ...
            nodePath.replaceWithMultiple([
                thingExported,
                state.types.expressionStatement(state.types.assignmentExpression('=', state.types.memberExpression(state.types.identifier("exports"), state.types.identifier("default")), thingExported.id))
            ]);
        }
        else {
            // Anon func
            var expr = toExpression(thingExported, state);
            nodePath.replaceWith(state.types.assignmentExpression('=', state.types.memberExpression(state.types.identifier("exports"), state.types.identifier("default")), expr));
        }
    }
    ...
    var visitor = {
        Program: {
            exit(programPath, state) {
                ...
                var functionBody = state.types.blockStatement(programPath.node.body);
                var funcArgs = [
                ...
                */
                ...
                // __mm['Module/Path']
                state.types.memberExpression(state.types.identifier("__mm"), state.types.stringLiteral(modName.toLowerCase()), true), 
                // function(global,exports){
                state.types.functionExpression(null, funcArgs, functionBody));
...
function getNamespacedTSName(typeNameNode) {
...
var configuredMangleNames = [index_js_1.default, { exclude: { '_h': true } }];
var minifiedPlugin = createPlugin(true);
var nonMinifiedPlugin = createPlugin(false);
var tsPropsPlugin = createTsExportPlugin();
var presetsES8 = [
    [preset_env_1.default, { targets: { chrome: 90 }, modules: false }],
    [preset_react_1.default, { useSpread: true, pragma: '_h' }]
];
...
function transformES8(code, moduleName, fullModulePath, opts) {
    ...
    var src = core_1.default.transformSync(code, {
        filename: moduleName,
        presets: presetsES8,
        caller: {
            name: 'es8'
        ...
        plugins: minified ? [
            ...
            [plugin_transform_typescript_1.default, { isTSX: true, optimizeConstEnums: true }],
...
exports.default = { transformES8, transformES8Json };
