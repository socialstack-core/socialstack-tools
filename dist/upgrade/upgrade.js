...
const { getModuleMap, getModuleIdMap, getModuleFilePath, replaceModule, installSingleModule, getOrCacheZip, tidyModuleName, deleteFolderRecursive, runCmd, prepareForUpgrade, searchForModules } = helpers_js_1.default;
            ...
            if (subModuleInfo.length > 0) {
                ...
                getModuleIdMap().then(moduleInfo => {
                    ...
                    mods.forEach(localModule => {
                        ...
                        var idealModulePath = path_1.default.join(config.projectRoot, getModuleFilePath(remoteModule));
                    ...
                    if (toUpgrade.length != 0) {
                        ...
                        var preparations = Promise.all(toUpgrade.map(localAndRemote => {
                        ...
                        return preparations.then(preparedModules => {
                            ...
                            if (localEdits) {
                                console.log('');
                                console.log(localEdits + " module(s) with newer versions have been edited in this project and therefore won't be upgraded.\r\n" +
...
function uninstallSubmodule(moduleName, config) {
    return new Promise((success, reject) => {
        ...
        runCmd('git submodule deinit -f "' + moduleFilePath + '"', config)
            ...
            var normalGitRoot = path_1.default.normalize(gitRoot).trim();
            var normalProjectRoot = path_1.default.normalize(config.projectRoot).trim();
            if (!normalGitRoot) {
                return;
            }
            var deltaDirectory = '';
            if (normalGitRoot != normalProjectRoot) {
                ...
                deltaDirectory = path_1.default.relative(normalGitRoot, normalProjectRoot) + '/';
            }
            // Meta exists?
            return new Promise((s, r) => {
                var realGitMetaPath = normalGitRoot + '/.git/modules/' + deltaDirectory + moduleFilePath;
                fs_1.default.stat(realGitMetaPath, function (err, stat) {
                    if (!stat) {
                        s();
                        return;
                    }
        ...
        list.forEach(function (file) {
            ...
            fs_1.default.stat(file, function (err, stat) {
                if (stat && stat.isDirectory()) {
                    ...
                    fs_1.default.readFile(file + '/HEAD', 'utf8', function (err, data) {
                        if (err && err.code == 'ENOENT') {
                            ...
                            if (data.startsWith('ref:')) {
                                ...
                                fs_1.default.readFile(file + '/' + reffing, 'utf8', function (err, data) {
                                    if (err) {
...
function runCmd(cmd, config) {
    return new Promise((success, reject) => {
        var proc = (0, child_process_1.exec)(cmd, {
            cwd: config.projectRoot
        }, function (err, stdout, stderr) {
            if (err) {
                // Fail:
