...
const { jsConfigManager, getLocalConfig, settingsPath } = configManager_1.default;
const helpers_js_1 = __importDefault(require("../install/helpers.js"));
const { installModules } = helpers_js_1.default;
const helpers_js_2 = __importDefault(require("./helpers.js"));
const { createDatabase, tidyUrl } = helpers_js_2.default;
...
exports.default = (config) => {
    ...
    function askFor(text, configName, cb) {
        return new Promise((success, reject) => {
            ...
            console.log(text);
            var rl = readline_1.default.createInterface(process_1.default.stdin, process_1.default.stdout);
    ...
    askFor('What\'s the public URL of your live website? Include the http or https, such as https://socialstack.cf', 'url').then(config => {
        // Set the root:
        tidyUrl(config);
    ...
    }).then(config => askFor('(Optional) Which modules would you like to install now? Separate multiple modules with , or press enter to skip', 'modules')).then(cfg => {
        console.log('Attempting to create a git repository via "git init"..');
        config.projectRoot = config.calledFromPath;
        return new Promise((s, r) => {
            (0, child_process_1.exec)('git init', {
    ...
    }).then(() => console.log('Complete. You can now run the project with "dotnet run" or start it with your favourite IDE.'));
};
