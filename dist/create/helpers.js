...
const { jsConfigManager, settingsPath, getLocalConfig } = configManager_1.default;
...
function createSiteAdmin(connection, config, success) {
    ...
    if (connection.type == 'mongodb') {
        ...
        connection.query(createAdminUser, function (err, results, fields) {
            if (err) {
                console.log(err);
                console.log('Error occurred whilst trying to setup an admin account for you. Skipping it.');
            }
...
function installDatabase(config) {
    ...
    if (localConfig == null || localConfig.databases == null || localConfig.databases.local == null) {
        return Promise.reject({ message: "Unable to create the database automatically as socialstack tools hasn't been configured with a database to connect to. " +
                "You can either:\r\n\r\n" +
                ...
                "This generate command reads the project's appsettings.json and turns the connection string in to executable SQL for you."
        });
    }
...
function createDatabase(connectionInfo, config) {
    return new Promise((success, reject) => {
        ...
        connection.query(sql[0], function (err, results, fields) {
            if (err) {
                connection.close();
