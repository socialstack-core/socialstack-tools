...
function loadFrontend(window, config, moduleSet) {
    ...
    try {
        // Go get the JS now:
        var jsFileContent = fs_1.default.readFileSync(jsFile, { encoding: 'utf8' });
        var __scope__ = {
            exports: window
        };
...
function getRenderer(config, moduleSet) {
    ...
    try {
        ...
        try {
            appSettings = require(config.projectRoot + '/Api/appsettings.json');
        }
        catch (e2) {
            console.log(e2);
        }
    ...
    var window = {
        fetch: (...args) => {
            ...
            var promise = (0, node_fetch_1.default)(...args);
    ...
    function renderWithFetch(canvasInstance, success) {
        ...
        // Attempt to render:
        var html = (0, preact_render_to_string_1.default)(canvasInstance);
